...
class Controller(object):
    def __init__(self, vehicle_mass, fuel_capacity,brake_deadband, decel_limit, accel_limit, wheel_radius, wheel_base,
                                     steer_ratio , max_lat_accel, max_steer_angle,kp, ki, kd,min_throttle,max_throttle ):
    ...
    def control(self, current_vel,dbw_enabled,linear_vel, angular_vel):
        ...
        sample_time = current_vel - self.last_velocity
        self.last_time = current_time

        throttle = self.throttle_controller.step(vel_error,sample_time)
        brake = 0

        if 0 == linear_vel and vel_error < 0.1:
            throttle = 0
            brake = 700 # Carla needs 700 N*m to stop
